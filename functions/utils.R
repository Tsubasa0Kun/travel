##-- Caixas ----
box_format <- function(texto, cor){
  HTML(paste('<div class = "box_format" style = "border:1px solid', 
             cor, '; background-color: ', 
             cor, ';">', 
             h3(texto), '</div>'))
}

tab_format <- function(texto, cor, icon, id){
  HTML(paste0('<a id="', id,'" href="#" class="action-button">
                  <div class = "style-block" style = "background-color:', cor, ';"> 
